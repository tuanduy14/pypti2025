...
def delete_schedules(schedules):
    ...
    print("\nDelete options:")
    print("1. Delete by Day")
    print("2. Delete by Course")
    print("3. Delete All Schedules")
    choice = int(input("Select an option: "))

    if choice == 1:
        day = input("Enter Day to delete: ")
        schedules = [s for s in schedules if s.Day.lower() != day.lower()]
        print(f"Schedules on {day} deleted successfully!")
    elif choice == 2:
        course = input("Enter Course to delete: ")
        schedules = [s for s in schedules if s.Course.lower() != course.lower()]
        print(f"Schedules for course {course} deleted successfully!")
    elif choice == 3:
        schedules.clear()
...
def view_scores(scores):
    ...
    if choice == 1:
        ...
        if filtered_scores:
            table = [[s.student_id, s.name, s.subject, s.score] for s in filtered_scores]
            headers = ["Student ID", "Name", "Subject", "Score"]
            print(tabulate(table, headers, tablefmt="oiklilgrid"))
            input('\nPress Enter to continue.\n')
        else:
