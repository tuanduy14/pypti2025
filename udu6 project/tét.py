...
def show_menu_students():
    ...
    print("| Option 2: Scores                |")
    print("| Option 3: Exit.                 |")
    print("-----------------------------------")


class Student_Schedule:
    def __init__(self, Day,Start,End,Subject):
        self.Day=Day
        self.Start=Start
        self.End=End
...
def opt_1_stu():
    ...
    calendar = Student_Schedule(Day, Start, End, Subject)

    return calendar


def opt_1_stu():
    print("------|Creative to calender|-------")
    print("| Option 1: Creative to calender.|")
    print("| Option 2: Show calender.        |")
    print("| Option 3: Update calender.     |")
    print("| Option 4: Save to Exit.         |")
    print("-----------------------------------")
...
def read_calendar_to_csv():
    ...
    try:
        with open(FILE_NAME_4, mode="r", newline='', encoding="utf-8") as file:
            ...
            for row in reader:
                if len(row) == 4:
                    calendars.append(Student_Schedule(row[0], row[1], row[2], row[3]))
    except FileNotFoundError:
        print(f"{FILE_NAME_4} not found, starting with an empty calendar.")
    return calendars


def update_calendar(calendars):
    in_calendar_to_table(calendars)
    if not calendars:
        print("No calendars to update")
        return calendars

    index = select_in_range("Enter a update schedule (1 - " + str(len(calendars)) + "): ", 1, int(len(calendars))) - 1

    print("------|Update Calenders|-----")
    print("| 1. Update Day            |")
    print("| 2. Update Start Time     |")
    print("| 3. Update End Time       |")
    ...
    choice = select_in_range("Select an option (1 - 4): ", 1, 4)
    if choice == 1:
        calendars[index].Day = input("Enter new day: ")
    elif choice == 2:
        calendars[index].Start = input("Enter new start time: ")
    elif choice == 3:
        calendars[index].End = input("Enter new end time: ")
    elif choice == 4:
        calendars[index].Course = input("Enter new course: ")
...
# Menu display for score management
def show_score_menu():
    ...
    print("| Option 5: Save Scores to File          |")
    print("| Option 6: Load Scores from File        |")
    print("| Option 7: Exit                         |")
    print("-----------------------------------------")
...
# Function to view scores with filters
...
def load_scores_from_file():
    ...
    try:
        ...
        print(f"No saved scores found in {FILE_NAME_5}. Starting fresh.")
    except Exception as e:
            ...
            while True:
                ...
                while True:
                    if choice_stu == 1:
                       ...
                       choice_ex_sche = select_in_range("Select an option (1 - 4): ", 1, 4)
                       if choice_ex_sche == 1:
                           calendars = input_calendar()
                       try:
                           if choice_ex_sche == 2:
                               in_calendar_to_table(calendars)
                           elif choice_ex_sche == 3:
                               calendars = update_calendar(calendars)
                           elif choice_ex_sche == 4:
